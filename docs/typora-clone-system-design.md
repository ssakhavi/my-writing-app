...
| Diagrams — text-syntax (Mermaid/D2/etc.) | Not included in v1 | Include Mermaid for Typora parity | Keeps bundle size and scope down per the PRD's anti-PKM stance; still deferred — see `typora-clone-mermaid-alternatives.md` |
| Diagrams — freeform drawing | Standalone tldraw panel, flattened to a PNG on insert (added 2026-07-20, ahead of Milkdown) | Inline re-editable tldraw blocks (store scene JSON in/alongside the doc) | Flattening to an image sidesteps document-format questions (where does tldraw scene state live?) and works before the real editor exists; loses re-editability, which is the noted trade-off to revisit once Milkdown lands |

## 5.1 Diagram panel (added 2026-07-20, ahead of the Milkdown spike)
...
- **If text-syntax diagrams are added later**: revisit the D2 recommendation from `typora-clone-mermaid-alternatives.md` and re-measure actual bundle-size/cold-start impact before committing.
- **When Milkdown lands**: reconcile the diagram panel's placeholder textarea with the real editor, and move diagram storage to a doc-relative `assets/` folder (see 5.1).
