...
- [x] ~~a "Format" group (Bold/Italic/Strikethrough/Inline Code)~~ — added, then removed at the user's request. The `buildMenu`/`commandsCtx`/toggle-mark-command wiring from that attempt is gone from `MilkdownEditor.tsx`.
...
- [x] impl: hide the slash menu's scrollbar. Crepe caps the item list at 420px and scrolls past that (`node_modules/@milkdown/crepe/lib/theme/common/block-edit.css`), which shows a visible scrollbar track/thumb — out of place in this app's minimalist aesthetic, and what "remove the slider" actually referred to (an earlier session wrongly read that as "remove the Divider item," which was also done and left in place since nothing indicated it should be reverted). Fixed with a CSS-only override in `src/styles.css` (`scrollbar-width: none` / `::-webkit-scrollbar { display: none }`, `!important` since Crepe's own stylesheet — injected via JS import — loads after this plain `<link>`-based one and would otherwise win the cascade at equal specificity). Scrolling still works, it's just not rendered.
